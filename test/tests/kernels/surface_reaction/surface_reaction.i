...
[Functions]
  [ic_func_eta_f]
    type = ParsedFunction
    #Vishal value = '0.5^2*(1.0-tanh(2*(x-x_val)/(int_width)))*(1.0-tanh(2*(y-y_val)/(int_width)))'
    expression = '0.5^2*(1.0-tanh(2*(x-x_val)/(int_width)))*(1.0-tanh(2*(y-y_val)/(int_width)))'
    #Vishal vars = 'int_width  x_val    y_val'
    symbol_names = 'int_width  x_val    y_val'
    #Vishal vals = '4644       23220    232200' # int_width = 1 micron (half total)
    symbol_values = '4644       23220    232200' # int_width = 1 micron (half total)
  []
  [ic_func_eta_g]
    type = ParsedFunction
    #Vishal value = '1 - 0.5^2*(1.0-tanh(2*(x-x_val)/(int_width)))*(1.0-tanh(2*(y-y_val)/(int_width)))'
...
[Kernels]
  ...
  [AC_f_sw]
    ...
    hj_names = 'h_f     h_g'
    mob_name = L
    #Vishal args = 'w_c w_o w_co eta_g T'
...
[Materials]
  ...
  [omega_f]
    ...
    material_property_names = 'To k_b Va Ef_o_f Ef_co_f Ef_v tol'
  []
