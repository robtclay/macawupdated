...
[Materials]
  ...
  [Aca_params]
    ...
    prop_names  = 'B                       h                                  pi'
    prop_values = '${fparse 1e-17*lo^2}    ${fparse 4.1356677e-15*to/eo}      3.14159265359'
  []
  #----------------------------------------------------------------------------#
  [M_params]
    type = GenericConstantMaterial

    prop_names  = 'mO                   mN                  mO2                     mCO                             mCO2'
    prop_values = '${fparse 15.999/Av}  ${fparse 14.01/Av}  ${fparse 15.999*2/Av}   ${fparse (12.011+15.999)/Av}    ${fparse (12.011+15.999*2)/Av}'
  []
  #----------------------------------------------------------------------------#
  [P_params]
    type = GenericConstantMaterial

    prop_names  = 'pO   pO2   pN'
    prop_values = '2.4  2.4   2.4'
  []
  #----------------------------------------------------------------------------#
  # Interface Width
  [int_width]
  ...
  [F_O]
    ...
    expression = '0.25 * sqrt(8*k_b*T/(pi*mO))'

    material_property_names = 'k_b pi mO'
  []
  ...
  [params]
    ...
    prop_values = '3000  ${fparse 8.6173e-5/eo}  1.0'
  []
...
#------------------------------------------------------------------------------#
[Outputs]
  file_base = ./results/step2_multi_aca_out
  [check]
    type = Checkpoint
    num_files = 4
  []
